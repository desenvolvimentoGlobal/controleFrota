...
**Regras definidas na revisão de 23/09/2026 (confirmadas pelo usuário em 23/09/2026):**
...
| 23/09/2026 | Revisão de bugs (fases 0–4) | Segurança: gestor não promove a admin nem tira gente da cadeia (validação no servidor), não edita admin, admin não exclui a si mesmo, fotos de checagem só para quem pode ver, usuário inativado perde a sessão. Estados: saída só com veículo disponível/reservado, sem sistema crítico e dentro do período; rascunho de alocação cancelada não conclui mais; cancelar não libera veículo em uso; retorno e fim de manutenção voltam a "reservado" se houver saída hoje e a "indisponível" se houver bloqueio; bloqueio pedido com carro na rua vale no retorno; manutenção não ressuscita veículo baixado nem desfaz indisponível manual; mudança manual não mexe em veículo em uso/reservado/manutenção; dupla conclusão da checagem travada. Rotina `alocacoes:sincronizar` (15 min) reserva no dia, expira aprovadas que não saíram e marca atrasos. Admin encerra alocação sem checagem. Fuso America/Sao_Paulo. Relações com `withTrashed` (excluir veículo/usuário não quebra telas). Infra: BOM removido de deploy.sh/Dockerfile/compose, worker roda `schedule:work`, volume de fotos públicas montado no nginx. 15 testes novos (43 no total). | Fase 5. |
| 23/09/2026 | 5 — Integrações e deploy | API `/api/integracao/v1` somente leitura (veículos, veículos disponíveis num intervalo, agenda com escopo `alocacoes`), tokens com hash e escopos, comando `integracao:token`, envelope de erro para qualquer falha em `api/*`, limite de 120 req/min; consumo do gestaoPessoas com vínculo manual da ficha no cadastro do usuário, sincronização diária (06:00) e botão "Sincronizar com o RH" (nome, setor, cargo, desligado → inativo, exceto quem está com carro); PWA instalável (manifest, ícones, service worker sem cache); deploy: `deploy.sh` corrigido para `.env.docker.example`, `docs/DEPLOY.md` e `docs/API.md`; caches de produção (config/route/view) testados; 6 testes novos (49 no total). | Consumidor no emissaoOS. |
| 23/09/2026 | 5 — Consumo pelo emissaoOS | No emissaoOS (branch `melhorias`, commit `fb3fdac`): cliente `App\Integracoes\ControleFrota`, `VeiculosDaFrota` no planejamento de instalação (catálogo com os veículos da frota primeiro e avisos de veículo parado ou alocado no mesmo dia/período, sem bloquear), sistema `controle-frota` no `integracao:token`. Ligação local validada ponta a ponta: emissaoOS (8001) → Frota (8005) com token de escopo `alocacoes`; Frota → gestaoPessoas (8002) lendo 64 colaboradores. Regras 8–11 confirmadas. | Deploy real no servidor (com o usuário) e vincular as fichas do RH aos usuários. |
| 23/09/2026 | Revisão geral (sistema inteiro) | Quatro revisões independentes (segurança, alocação/checagem, manutenção/custos, telas/JS) e varredura de todas as telas GET com os quatro perfis. Corrigido: saída antecipada não leva carro reservado para outro no mesmo dia; conclusão da checagem e criação/aprovação de alocação travam o veículo (sem reserva duplicada em clique duplo); km da saída com salto > 500 km é recusado e o gestor corrige km para baixo pelo ajuste manual (não com o carro em uso); retenção nunca apaga a última checagem de cada veículo; solicitação não aprovada expira e período já encerrado é recusado; "1.500" era gravado como R$ 1,50; editar manutenção não apaga fornecedor/responsável inativo nem se põe como responsável; indisponível manual sobrevive a manutenção com bloqueio; liberar à mão veículo bloqueado por manutenção e baixar com manutenção aberta são recusados; preventiva aberta à mão liga ao plano vencido e cancelada não reabre por 30 dias; validade no último dia não é "vencida" e aviso repetido não vibra o celular todo dia; Excel sem fórmula injetada; placa/chassi de veículo excluído podem ser reusados; Web Push só aceita endpoints dos serviços de push (SSRF) e "Sair" desinscreve o aparelho; gestor não dá perfil financeiro e gestor direto precisa ser gestor/admin; troca de senha derruba as outras sessões; foto de usuário recusada não apaga a antiga; API de agenda inclui carro em uso atrasado; exclusão de fornecedor só admin; histórico de checagens só admin/gestor; erros da checagem por foto aparecem (não "Sem conexão") e o mesmo item não é enviado duas vezes; erros de validação em modal viram toast; checkbox desmarcado não volta marcado após erro; filtros lembrados não perdem ?editar; texto do data-gc-confirm aparece; clique duplo em formulário POST ignorado. 13 testes novos (62 no total). | Decisões pendentes do usuário (ver relatório): motorista com duas alocações simultâneas; o que o perfil geral vê na ficha do veículo e na agenda. Deploy adiado pelo usuário. |
| 23/09/2026 | Preparação do deploy | Infra comparada com o `contasReceber` (deploy de 22/09/2026, o mais confiável) e erros herdados do template corrigidos: seeder de produção criava `admin`/`senha123` (agora não cria usuário; novos comandos `usuario:criar` e `usuario:senha`, com senha às cegas, régua única `Password::defaults()` e auditoria); domínio do Traefik fixo como `SEU-IP` no `compose.yaml` (agora `TRAEFIK_HOST` no `.env`); `INSTALL_DEV=true` na imagem de produção (agora só no dev); `.dockerignore` deixava entrar as fotos locais e o link `public/storage`; `.env.docker.example` com `SESSION_SECURE_COOKIE=false`, `APP_NAME` "frota" e sem a URL do RH de produção; Nginx com cabeçalho do gestaoPessoas, sem `try_files` no PHP nem timeout para PDF/Excel; `deploy.sh` confere o `worker` (o agendador daqui). Já estavam certos: resolver do Nginx (502 ao recriar o `app`), bit 100755 do `deploy.sh`, LF, `forceScheme`. `docs/DEPLOY.md` virou `docs/SERVIDOR-SETUP.md`, roteiro passo a passo no formato do contasReceber (o antigo mandava editar o `compose.yaml` no servidor, testava em `http://localhost` e prometia `admin`/`senha123`). 4 testes novos (66 no total). | Deploy. |
| 23/09/2026 | **Em produção** | Deploy feito pelo usuário seguindo `docs/SERVIDOR-SETUP.md`: https://frota.18-231-84-211.nip.io. | Melhorias. |
| 23/09/2026 | Melhoria: CPF opcional | CPF deixa de ser obrigatório no cadastro de usuário (tela e `usuario:criar`); se informado, continua validado e único (migration `cpf_do_usuario_opcional`, coluna nullable, unique mantido). Corrigido junto: a busca de usuários por nome (sem dígitos) virava `cpf like '%%'` e trazia todo mundo. 2 testes novos (68 no total). | Próxima melhoria. |
