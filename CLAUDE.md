...
0 Fundação ✔ · 1 Cadastros ✔ · 2 Alocação e checagem ✔ · 3 Manutenção ✔ · 4 Financeiro ✔ · Revisão ✔ · 5 Integrações e deploy ✔ · **em produção desde 23/09/2026** (https://frota.18-231-84-211.nip.io). Agora: melhorias pedidas pelo usuário. Produção = migrations só para frente.
...
Regras da fase 5: API em `routes/api.php` (`/api/integracao/v1`), só GET, autenticada por `integracao.api[:escopo]` (tokens em `tokens_integracao`, comando `integracao:token`); tudo que sai para outro sistema passa por `App\Services\Integracao\VeiculoParaIntegracao` (serialização explícita, nunca `toArray()`); respostas por `App\Support\RespostaApi` e erros de `api/*` no mesmo envelope (bootstrap/app.php). Consumo do RH por `App\Integracoes\GestaoPessoas` (timeout curto, falha silenciosa, `null` = falhou); vínculo manual por `usuarios.colaborador_externo_id` porque a API do RH não expõe CPF/e-mail. PWA: `public/manifest.json`, ícones `images/icone-pwa-*.png`, `public/sw.js` sem cache de páginas. Contrato em `docs/API.md`, deploy em `docs/SERVIDOR-SETUP.md`.

Regras da revisão geral (23/09/2026): a checagem de saída antes do horário previsto exige que ninguém tenha o carro entre agora e a saída (`Alocacao::conflitantes`); `ChecagemService::concluir` e `AlocacaoService::solicitar/aprovar` fazem `lockForUpdate` no veículo e validam DENTRO da transação; km da saída acima de `frota.checagem.km_tolerancia_saida` é recusado e só o ajuste manual (`atualizarKm` com origem `manual`) pode baixar km, nunca com o carro em uso; a retenção preserva a última checagem concluída de cada veículo; "indisponível manual" = última mudança de situação fora das manutenções feita à mão (`ManutencaoService::liberarVeiculo`); dinheiro digitado passa por `Numero::dePtBr` ("1.500" = 1500); planilhas usam `TextoSemFormula`; placa/chassi únicos só entre não excluídos (`withoutTrashed`); notificação com `atualizarNaoLida` e mesmo texto não repete o push (semana); Web Push só aceita hosts de `WebPushController::HOSTS_DE_PUSH`; gestor só atribui perfis geral/gestor e `gestor_id` precisa ser gestor/admin; sessões caem na troca de senha (`SessaoInvalidadaPelaSenha`); fetch com `Accept: application/json` recebe erros em JSON (`shouldRenderJsonWhen`); checkbox em form de edição usa `session()->hasOldInput() ? old(x) : $model->x`.
